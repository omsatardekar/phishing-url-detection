...
import tldextract  # Extract domain name from URL
import dns.resolver  # To check if domain has valid DNS records
...
app.config['SECRET_KEY'] = 'c18eb146d7303b95b5a199a43510441bb06e7adbfab6c1701b2d4634dac08e45'
...
def is_trusted_domain(url):
    ...
    try:
        dns.resolver.resolve(domain, 'A')  # Check if DNS records exist
        return True  # If DNS record is successful, domain is valid
    except:
        return False  # If no valid DNS record, domain might be suspicious
...
def predict():
    try:
        ...
        url_exists = is_url_valid(url)

        # Step 3: Apply Domain Trustworthiness Check
        if not url_exists and not is_trusted_domain(url):
            return jsonify({
                'url': url,
                'is_malicious': True,
                'confidence': 100.0,
                'prediction': "Phishing (Non-Existent & Untrusted Domain)",
                'status': 'success'
            })

        # Step 4: Extract Features & Model Prediction
...
# Secret key for session management
app.secret_key = 'c18eb146d7303b95b5a199a43510441bb06e7adbfab6c1701b2d4634dac08e45' 
